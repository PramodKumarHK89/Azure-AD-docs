# Embedding extension attribute as a claim in the token.
...
- The app object ID from the previous step can be found in the app registration blade. Below is the screenshot for reference.
...
- After connecting to Azure AD with Connect-AzureAD, If you run the command Get-AzureADPolicy and PowerShell prompts you with the message "Get-AzureADPolicy : The term 'Get-AzureADPolicy' is not recognized". Adding the -AllowClobber argument will fix the installation problem.
